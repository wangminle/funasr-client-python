...
5. 所有代码都放在/src目录中；所有代码都使用中文注释；
6. 如果开发过程中有任何需要输出到本地文件的，都输出到/dev/output目录中；
7. 如果有任何日志要记录在本地，都记录到/dev/logs目录中；
8. 如果需要有任何本地配置文件，都放在/dev/config目录中；
...
5. 本项目中架构设计文档，需求规格说明书，UI设计说明文档以及项目管理文档，这四个文档是重要文档，可以在文档前增加项目前缀："funasr-python-gui-client-v3-"。
6. V3版本的核心文档位于/docs/v3目录中，包括技术实施方案和项目进展总结。
...
2. V3版本核心特性：协议适配层（解决新旧服务端is_final语义差异）、服务探测器（自动检测服务器能力）、配置持久化（V2→V3自动迁移）。
3. 核心模块：protocol_adapter.py（协议适配）、server_probe.py（服务探测）、websocket_compat.py（WebSocket兼容层）、config_utils.py（配置工具）。
